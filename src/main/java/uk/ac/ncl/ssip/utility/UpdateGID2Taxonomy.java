 ...
 * @author nmc91
 */
public class UpdateGID2Taxonomy {
    ...
    public static void main(String[] args) {
        ...
        try {
            String fileroot = "/home/ec2-user/cbm-protease/data/taxonomy/";//"/usr/local/apache2/htdocs/cbm-protease/data/";
            ...
            for (File file : filefolder.listFiles()) {
                ...
                //create gene ID to taxonomy map 
                Map<String, String> geneid2taxonomy = new HashMap<String, String>();
                for (char ch = 'a'; ch < 'd'; ch++) {
                    File annotationFile = new File(fileroot + "taxonomy/" + file.getName().substring(0, 3) + ".annotations.xa"+ch+".taxon");
                    Scanner taxScan = new Scanner(annotationFile);
                    while (taxScan.hasNextLine()) {
                        String str = taxScan.nextLine();
                        String[] splits = str.split("\t");
                        if(splits[3].equals("tax name not found")){
                        geneid2taxonomy.put(splits[0], splits[3]);
                        }
                    }
                }
                Map<String, Set<String>> geneconfig2taxonomy = new HashMap<String, Set<String>>();
                for (String configStr : geneconfig2geneids.keySet()) {
                    Set<String> taxons = new HashSet<String>();
                    for (String geneid : geneconfig2geneids.get(configStr)) {
                        taxons.add(geneid2taxonomy.get(geneid));
                    }
                    geneconfig2taxonomy.put(configStr, taxons);
                    System.out.print("\n" + configStr);
                    for (String str : geneconfig2taxonomy.get(configStr)) {
                ...
                FileWriter output = new FileWriter(new File(fileroot + "/final-gene-tables/" + file.getName() + ".taxon.csv"));
                BufferedWriter outputbuffer = new BufferedWriter(output);
                FileWriter output2 = new FileWriter(new File(fileroot + "/final-gene-tables/" + file.getName() + "-v2.taxon.csv"));
                ...
                for (String config : geneconfig2geneids.keySet()) {
                    ...
                    for (String str : geneconfig2taxonomy.get(config)) {
                        outputbuffer.write(str + " ");
                    }
                    outputbuffer.write("\n");

                    //add taxonomy for each gene id 
                    int[] hits = new int[86];
